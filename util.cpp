...
const char * const HEPTAFRANC_CONF_FILENAME = "heptafranc.conf";
const char * const HEPTAFRANC_PID_FILENAME = "heptafrancd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "heptafrancd -foo=bar
...
#else
    const char* pszModule = "heptafranc";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HeptaFranc
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HeptaFranc
    // Mac: ~/Library/Application Support/HeptaFranc
    // Unix: ~/.heptafranc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HeptaFranc";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HeptaFranc";
#else
    // Unix
    return pathRet / ".heptafranc";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", HEPTAFRANC_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", HEPTAFRANC_PID_FILENAME)));
}
